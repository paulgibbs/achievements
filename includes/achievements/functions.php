...
/**
 * Handles the leaderboard rankings
 *
 * Get the rank, User ID and points of all the users
 *
 * @param int $offset Optional The pagination offset.
 * @param int $posts_per_page Optional The number of rankings to be displayed per page.
 *
 * @since Achievements (3.4)
 */
function dpa_get_id_and_rank( $offset = 0, $posts_per_page = 0 ){
	global $wpdb, $dpa_leaderboard_max_pages;
	...
	FIND_IN_SET( karma.meta_value, ( SELECT  GROUP_CONCAT( DISTINCT ranking.meta_value ORDER BY CONVERT( ranking.meta_value,signed )  DESC) FROM {$wpdb->prefix}usermeta AS ranking WHERE ranking.meta_key = '{$wpdb->prefix}_dpa_points' ) ) as rank, ID, karma.meta_value
	FROM {$wpdb->prefix}users AS person
	LEFT JOIN {$wpdb->prefix}usermeta as karma
	ON person.id = karma.user_id
	AND karma.meta_key = '{$wpdb->prefix}_dpa_points'
	ORDER BY CASE WHEN rank IS NULL THEN 1 ELSE 0 END, rank";

	if ( $posts_per_page !== -1 ) {
		$query .= ' LIMIT %d, %d';
		$query = $wpdb->prepare($query, $offset, $posts_per_page);
	}
	$leaderboard_rank = $wpdb->get_results($query);
	$found_posts = (int) $wpdb->get_var( 'SELECT FOUND_ROWS()' );

	// Set the relevant query parts
	if ($posts_per_page !== -1) {
		$dpa_leaderboard_max_pages = ceil( $found_posts / $posts_per_page );
	} else {
		$dpa_leaderboard_max_pages = 1;
	}
